...
import { addManagerComplaint, updateManagerComplaint } from "@/utils/managerComplaintSlice"
...
const ComplaintDetailsManager = () => {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      dispatch(updateManagerComplaint(res.data.complaint))
      toast({
        title: "Success",
        description: "Complaint accepted successfully",
      })
    } catch (err) {
      console.error("Error accepting complaint:", err)
      toast({
        title: "Error",
        description: "Failed to accept complaint",
        variant: "destructive",
      })
    } finally {
  ...
  const handleResolve = async () => {
    ...
    try {
      const res = await axios.put(BASE_URL + `/complaints/${_id}/resolve`, {}, { withCredentials: true })
      dispatch(updateManagerComplaint(res.data.complaint))
      toast({
        title: "Success",
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={complaint?.status === "in progress" || complaint?.status === "resolved" || actionLoading.accept}
          ...
          <Button
            ...
            disabled={complaint?.status !== "in progress" || actionLoading.resolve}
            variant="outline"
          >
...
export default ComplaintDetailsManager
